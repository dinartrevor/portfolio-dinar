...
import { Edit, Trash, Plus, Heart } from 'lucide-react';

export default function Index({ hobbies }) {
    const { delete: destroy } = useForm();

    const handleDelete = (id) => {
        if (confirm('Are you sure you want to delete this hobby?')) {
            destroy(route('admin.hobbies.destroy', id));
        }
    };
        ...
        <AuthenticatedLayout
            header={
                <div className="flex justify-between items-center">
                    ...
                    <Link
                        href={route('admin.hobbies.create')}
                    ...
                    <div className="overflow-hidden bg-white shadow-xl sm:rounded-2xl dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
                        <div className="p-0 md:p-6 text-gray-900 dark:text-gray-100">
                            ...
                            <div className="md:hidden divide-y divide-gray-200 dark:divide-gray-700">
                                {hobbies.map((hobby) => (
                                    <div key={hobby.id} className="p-6 space-y-4">
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <div className="font-black text-gray-900 dark:text-white leading-tight">{hobby.name}</div>
                                                    <div className="text-xs text-gray-500 font-bold uppercase tracking-widest">Order: {hobby.order}</div>
                                                </div>
                                            </div>
                                            <div className="text-right">
                                                <span className={`px-2 py-1 text-[10px] font-black uppercase tracking-widest rounded-full ${hobby.is_active ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}`}>
                                                    {hobby.is_active ? 'Active' : 'Inactive'}
                                                </span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex justify-end gap-3 pt-2">
                                            <Link
                                                href={route('admin.hobbies.edit', hobby.id)}
                                ...
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="border-b border-gray-200 dark:border-gray-700">
                                            <th className="px-6 py-5 font-black uppercase text-[10px] text-gray-400 tracking-[0.2em]">Hobby Name</th>
                                            <th className="px-6 py-5 font-black uppercase text-[10px] text-gray-400 tracking-[0.2em]">Order</th>
                                            <th className="px-6 py-5 font-black uppercase text-[10px] text-gray-400 tracking-[0.2em]">Status</th>
                                            <th className="px-6 py-5 font-black uppercase text-[10px] text-gray-400 tracking-[0.2em] text-right">Actions</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                        {hobbies.map((hobby) => (
                                            <tr key={hobby.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/30 transition-colors group">
                                                <td className="px-6 py-5">
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <div className="font-black text-gray-900 dark:text-white text-lg tracking-tight">{hobby.name}</div>
                                                    </div>
                                                </td>
                                                <td className="px-6 py-5">
                                                    <span className="text-sm font-black text-gray-500">{hobby.order}</span>
                                                </td>
                                                <td className="px-6 py-5">
                                                    <span className={`px-3 py-1 text-xs font-bold uppercase tracking-widest rounded-full ${hobby.is_active ? 'bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 border border-green-200 dark:border-green-800/50' : 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-800/50'}`}>
                                                        {hobby.is_active ? 'Active' : 'Inactive'}
                                                    </span>
                                                </td>
                            ...
                            {hobbies.length === 0 && (
                                <div className="py-24 text-center">
                                    ...
                                    <p className="text-gray-500 font-bold uppercase tracking-widest text-xs">No hobbies documented yet.</p>
                                </div>
                            )}
                        </div>
                    </div>
